...
def findEven(arr):
    ...
    if arr[0] % 2 == 0:
        # add element to array also containing results of recursive call to findEven()
        ...
        return [arr[0]] + findEven(arr[1:])
    else:
        # if mod 2 == 0
        # do not add element to array and continue by making recursive call to findEven()
        # pass array with 1st element removed
        # continue until base case reached
        # return nested results back up the chain of recursive calls
        return findEven(arr[1:])
